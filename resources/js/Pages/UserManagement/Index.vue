...
const deleteUser = () => {
    if (userToDelete.value) {
        useForm().delete(route("user-management.delete-user", userToDelete.value.id), {
            onSuccess: () => {
...
const deleteInvitation = () => {
    if (invitationToDelete.value) {
        useForm().delete(route("user-management.delete-invitation", invitationToDelete.value.id), {
            onSuccess: () => {
    ...
    <AdminLayout :user="$page.props.auth.user">
        <div class="space-y-6">
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <p class="mt-2 text-gray-600">Administra usuarios, roles e invitaciones del sistema</p>
            </div>
            ...
            <div class="bg-white shadow rounded-lg p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-medium text-gray-900">Invitar Nuevos Usuarios</h3>
                        <p class="text-sm text-gray-600">Envía invitaciones a profesores y padres</p>
                    </div>
                    <button
                    ...
                    <form @submit.prevent="submitInvite" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                />
                                <div v-if="inviteForm.errors.email" class="text-red-500 text-sm mt-1">
                            ...
                            <div>
                                ...
                                <div v-if="inviteForm.errors.role_id" class="text-red-500 text-sm mt-1">
            ...
            <div class="bg-white shadow rounded-lg">
                ...
                <div class="p-6">
                    <div v-if="invitations.length === 0" class="text-gray-500 text-center py-4">
                        No hay invitaciones pendientes
                    </div>
                    <div v-else class="space-y-4">
                        <div v-for="invitation in invitations" :key="invitation.id" class="border rounded-lg p-4 flex justify-between items-center">
                            <div>
                                <p class="font-medium">{{ invitation.name }}</p>
                                <p class="text-sm text-gray-600">{{ invitation.email }}</p>
                                <p class="text-sm text-gray-500">{{ invitation.role.display_name }}</p>
            ...
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    ...
                    <div class="flex justify-center space-x-3 mt-4">
                        <button @click="cancelDelete" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-md text-sm font-medium transition-colors">
                            Cancelar
                        </button>
                        <button @click="deleteUser" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">
                            Eliminar
                        </button>
                    </div>
                </div>
        ...
        <div v-if="showDeleteInvitationModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-red-100">
                        <svg class="h-6 w-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-500">
                            ¿Está seguro de que desea eliminar la invitación para
                    ...
                    <div class="flex justify-center space-x-3 mt-4">
                        ...
                        <button @click="deleteInvitation" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">
                            Eliminar
                        </button>
                    </div>
                </div>
